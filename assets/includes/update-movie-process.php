...
} else if (empty(trim($name)) || empty(trim($year))) {
...
} else if ($common->notExists("movie", "name", $common->getPretty(trim($name)), $id)) {
	$errors[] = "<b>" . $name . "</b> is already exists in database!";
} else {

	if (empty($movie_list) && empty($tv_series_list)) {

		if ($update->updateMovie($id, $common->getPretty(trim($name)), $genres, trim($year), $movie_list, $tv_series_list)) {
		...
		if ($update->updateMovie($id, $common->getPretty(trim($name)), $genres, trim($year), $movieIDArray, $tv_series_list)) {
			$errors[] = "<span class=\"success\"><b>" . $name . "</b> is updated!</span>";
		} else {
			$errors[] = "Sorry, something went wrong!";
		}
		...
		if ($update->updateMovie($id, $common->getPretty(trim($name)), $genres, trim($year), $movie_list, $tvSeriesArray)) {
			$errors[] = "<span class=\"success\"><b>" . $name . "</b> is updated!</span>";
		} else {
			$errors[] = "Sorry, something went wrong!";
		}
		...
		if ($update->updateMovie($id, $common->getPretty(trim($name)), $genres, trim($year), $movieIDArray, $tvSeriesArray)) {
			$errors[] = "<span class=\"success\"><b>" . $name . "</b> is updated!</span>";
		} else {
			$errors[] = "Sorry, something went wrong!";
		}
