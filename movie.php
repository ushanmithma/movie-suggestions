...
<head>
	...
	<title>Suggest a movie to watch - Cinema🇱🇰</title>
				...
				<div class="adding-form">
					<form method="post" id="adding-form">
						<div class="form-title">
							<h1>Suggest a <span>movie</span> to watch</h1>
						</div>
						<div class="form-group">
							<label for="name">Name:</label>
							<input list="movies" name="name" id="name" autocomplete="off" maxlength="255" autofocus>
							<datalist id="movies">
						...
						<div class="form-group">
							...
							<input list="years" name="year" id="year" autocomplete="off" maxlength="4">
							<datalist id="years">
	...
	<script>
		...
		$(document).ready(function(){
			...
			$('#adding-form').on('submit', function(event) {
				...
				$.ajax({
					url:"./assets/includes/validate-movie.php",
					method:"post",
					data:{
						name:name,
						genres:genres,
			...
			function load_name_list() {
				...
				$("datalist#movies option").each(function(){
					$(this).remove();
				});
				...
				$.ajax({
					...
					success: function(data){
						$.each(data, function(i){
							$("datalist#movies").append($("<option>").attr('value', data[i]));
						});
					}
				});
			}
